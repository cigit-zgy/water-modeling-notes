...
用户直接提供论文全文时，可按日期建立一期并直接进入全文评述，无须虚构摘要筛选历史或评分。此类条目的 aScore/fScore 均可省略；首页只提供本期范围及全文入口。篇幅以证据完整为准，不受下述周度短评字数限制。

学术中文是正文、标题、图注和表头的默认语言。逐一说明原文图表的研究问题、读法、观察、论点及边界；作者主张与评述者的迁移判断必须分开。源PDF可以显式裁定提取器错配，但原始提取不得被静默改写。缺失补充材料限定相关结论，不阻断有充分源证据的其他分析。

许可证或任务禁止转载时，不发布原图及近似复刻图；可以独立整理事实表和概念流程图。评述证据包保存在 `reports/paper-review/<日期>/<论文标识>/`，不进入网页集合，且不含源PDF或全文提取。通过权利门禁的原图单独归档到文章静态资源目录。原始科学材料保留在本地，公共证据包只记录相对定位、哈希与评述者的分析。

沿用预览设计的路径 `/paper-review/<日期>/papers/<slug>/`。草稿及其父期均须通过可见性门禁，正式发布在独立复审、构建和视觉验证后按任务的acceptance约束执行。
...
全文评述采用**叙事优先、证据中心**：研究矛盾 → 作者方案 → 方法如何工作 → 证据是否支持 → 从复现到开放问题 → 真正证明了什么 → 边界与局限 → 对当前研究的启发。这是论证逻辑，不是必须照抄的目录。复杂方法论文通常约 **4,000–7,000 中文字**，以连续论证和完整证据决定深度，不重复摘要或机械凑字数。

页面 H1 由 Layout 提供；正文只用 H2–H4。H2 表示完整叙事阶段，H3 表示科学问题、方法环节或证据组，H4 仅用于面板、技术细节或局部争议。不跳级，不用 H5 及更深层级。图表编号不得决定章节结构；主文图表全部解释，可在同一问题下组合多图，也可回访同一图。

每个关键对象应在自然段落中说明：为什么需要它、如何阅读、主要观察、支持何种主张、证据边界、评述判断。禁止把五个固定标签当作正文模板。作者主张、直接观察、评述推断分开；结构化 review 保持完整审计记录，博客用隐藏的 section data-evidence/data-claims 关联证据，不向读者显示内部编号。

有序列表用于步骤、因果链和判据；无序列表用于并列条件或局限。blockquote 仅用于短原文命题或明确标识的中心判断。AcademicCallout 使用 note/definition/method/caution，并显式给出中文 label。ResponsiveTable 用于归纳比较和数据；Mermaid 仅用于评述者抽象的流程或逻辑；代码块仅在接口、数据结构、伪代码本身构成解释对象时出现。不为装饰凑元素，不创建平行 CSS。

成稿后执行一次白名单式“去 AI 味”检查，参考 `larashero3-dotcom/lieflat-less-ai-tone` 的固定版本 `27d29232f10124db904ca9c0536d0b67cb3b2833`。这一步只修改明确命中的语言痕迹，例如翻案腔、空转式冒号、相邻句同构、段首零主语评论，以及原文已有具体数据却仍以抽象词概括的表达。不得借此重组章节、调整段落和列表、改变证据强度、增删事实或为了“更像人”而口语化。学术中文、证据完整性和项目写作规范优先于该第三方规则；未命中的文字保持原样。

#### 原图权利门禁

逐对象检查原图、图注和文章许可，记录源页面、下载URL、SHA-256、像素尺寸、第三方credit核验及使用条件。权利不明或存在排除声明时，自动不发布该图，保留文字证据解释。

CC BY-NC-ND 4.0 原图只用于非商业、未改编的转载：归档出版社完整原始图像字节，不增添标注、不改数据、不重着色、不拼接面板、不重绘近似版本。页面等比例缩放但不裁切，提供完整图链接。每图中文图注给出原作者、论文来源、DOI、许可链接及“原图未改编”；解释置于图外，不暗示作者背书。此门禁不自动覆盖商业用途或单独排除的第三方材料。
